...
{
    gpiohs_set_pin(SD_CS_GPIOHS_NUM, GPIO_PV_HIGH);
}

void SD_CS_LOW(void)
{
    gpiohs_set_pin(SD_CS_GPIOHS_NUM, GPIO_PV_LOW);
}

void SD_HIGH_SPEED_ENABLE(void)
{
    spi_set_clk_rate(SD_SPI, SD_SPI_CLK_HIGH_RATE);
}

static void sd_lowlevel_init(uint8_t spi_index)
{
	fpioa_set_function(SD_SCLK_PIN, FUNC_SPI1_SCLK);
	fpioa_set_function(SD_MOSI_PIN, FUNC_SPI1_D0);
	fpioa_set_function(SD_MISO_PIN, FUNC_SPI1_D1);
	fpioa_set_function(SD_CS_PIN, FUNC_GPIOHS0 + SD_CS_GPIOHS_NUM);

    gpiohs_set_drive_mode(SD_CS_GPIOHS_NUM, GPIO_DM_OUTPUT);
    spi_set_clk_rate(SD_SPI, 200000);     /*set clk rate*/
}

static void sd_write_data(uint8_t *data_buff, uint32_t length)
{
    spi_init(SD_SPI, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_send_data_standard(SD_SPI, SD_SPI_CS_NUM, NULL, 0, data_buff, length);
}
...
static void sd_read_data(uint8_t *data_buff, uint32_t length)
{
    ...
    spi_init(SD_SPI, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_receive_data_standard(SD_SPI, SD_SPI_CS_NUM, NULL, 0, data_buff, length);
...
{
    spi_init(SD_SPI, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_send_data_standard_dma(SD_SPI_DMA_CH, SD_SPI, SD_SPI_CS_NUM, NULL, 0, (uint8_t *)(data_buff), 128 * 4);
}

static void sd_read_data_dma(uint8_t *data_buff)
{
    spi_init(SD_SPI, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_receive_data_standard_dma(-1, SD_SPI_DMA_CH, SD_SPI, SD_SPI_CS_NUM,NULL, 0, data_buff,128 * 4);
}
